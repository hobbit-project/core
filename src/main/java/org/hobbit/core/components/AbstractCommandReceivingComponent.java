...
public abstract class AbstractCommandReceivingComponent extends AbstractComponent implements CommandReceivingComponent {
	...
	protected String createContainer(String imageName, String[] envVariables) {
		try {
			initResponseQueue();
			byte data[] = RabbitMQUtils.writeString(gson.toJson(new StartCommandData(imageName, "", containerId,
					envVariables)));
	 ...
	 * 
	 * @param containerId
	 *            the id of the container that should be stopped
	 */
	protected void stopContainer(String containerId) {
		byte data[] = RabbitMQUtils.writeString(gson.toJson(new StopCommandData(containerId)));
		try {
			sendToCmdQueue(Commands.DOCKER_CONTAINER_STOP, data);
		} catch (IOException e) {
			LOGGER.error("Got exception while trying to stop the container with the id\"" + containerId + "\".", e);
		}
	}
